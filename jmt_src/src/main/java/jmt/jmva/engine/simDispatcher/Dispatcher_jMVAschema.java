...
package jmt.jmva.engine.simDispatcher;
...
import jmt.engine.simDispatcher.ModelTransformer;
...
public class Dispatcher_jMVAschema {
	...
	public boolean solveModel() {
		...
		if (!transfSuccess) {
			...
			//copy the model with results (rename does not work!)
	...
	public ExactModel getTransformedModel(String inputModel, String outputModel) {
		...
		// region when it is not requested
		...
		for (int i = 0; i < input.getStations(); i++) {
			for (int j = 0; j < input.getClasses(); j++) {
				if (input.getVisits()[i][j] == 0
					...
					Iterator<Object> it = output.getMeasureKeys().iterator();
					while (it.hasNext()) {
						Object key = it.next();

...
public class SolverSingleClosedMVA extends Solver {
	...
	public SolverSingleClosedMVA(int cust, int stat) {
		...
		//OLD
	        //Not useful anymore: no longer extends SolverSingleClosedExact
		//super(cust, stat);
	 ...
	 * (so if you solve the model, you do not need to reorder results,
	 * they are reordered by the "solve" method itself; otherwise, if you
	 * do not solve the model after calling "input" method, you may have some problems).
	...
	/*
	    ...
	    //overrides indices() of SolverSingleClosedExact
	*/
	...
	public void solve(boolean intermediate) {
		...
		if (loadIndep) {
			...
			//TODO: add case intermediate=true for LD 
	...
	private void solveSingleLI() {
		...
		//the other indices dependent on totThroughput
		for (k = 0; k < stations; k++) {
	...
	private void solveSingleLI(boolean intermediate) {
		...
		if (!intermediate) {
			//intermediate results must not be saved, then use
			//the base method
			solveSingleLI();
			return;
		}
	...
	private void solveSingleLD() {
		...
		//it is not efficient must be changed
	...
	private void solveSingleLDErrCtr() {
		...
		//are available only for load dependent stations.
	...
	private void solveSingleLDErrCtrFaster() {
		...
		for (m = noLI ? 1 : LICount; m < stations; m++) {
			...
			for (n = 1; n <= customers; n++) {
				...
				//calculation of utilization & queue length of al m subsystem
				//from n
				for (int i = noLI ? 0 : LICount; i < m; i++) {
					nsCorr[i][n] = 0.0;
					usCorr[i][n] = 0.0;
	...
	private void solveSingleLDErrCtrFaster(boolean intermediate) {
            //TODO: still to fix, it does not yet give intermediate results
		if (!intermediate) {
			//intermediate results must not be saved, then use
			//the base method
			solveSingleLDErrCtrFaster();

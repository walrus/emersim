...
public class StatiDrawer extends JPanel implements Notifier/*, Runnable*/{
	...
	private int queueMax;//for animate 
	//				jobTmp;
	private int currentJob;
	private int lastJob;

	//panel settings
	private double panelH = 250; //minimum value for panelW
	...
	private Ellipse2D lastStatusE;
	private Ellipse2D transitionE;
	...
	//colors
	private Color probC;
	private Color queueProbC;
	private Color emptyC;
	private Color queueC;
	private Color animQueuesC;
	private Color animProbC;
	private Color animBorderC;
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < queueLength() + 1; i++) {
			...
			if ((queueLength() < queueMax) || (queueMax == 0)) { //queue is infinite or bigger than the visible area 
		...
		//animation
		if ((lastJob != currentJob) && (currentJob < queueLength() - 2) && animate) {
	...
	public void drawLastArc(boolean bold, String txt, Graphics2D g2d, Color c) {
		drawArc(queueLength() + 2, queueLength() + 1, false, "", g2d, c);
		drawArc(queueLength() + 1, queueLength() + 2, false, "", g2d, c);
	}
	...
	private int queueLength() {
		//int a = (int)((panelW - 2 * START_GAP  - 4 * (2 * STATUS_RAD + ELEMS_GAP) - ELEMS_GAP )/(2 * (2 * STATUS_RAD + ELEMS_GAP)));

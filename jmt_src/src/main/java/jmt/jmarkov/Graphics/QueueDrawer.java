...
public class QueueDrawer extends JComponent implements Notifier {
	...
	private double panelW = 400;

	private double minH = 100;

	private double minW = 400;
	...
	private double ELEM_HEIGHT;

	private double ELEM_WIDTH;

	private double ELEMS_GAP;

	private double PROC_RAD; 
	private static double MORE_JOBS_MAX = 100;

	private static double LOST_JOBS_MAX = 100;
	private Rectangle2D[] queue;
	private RoundRectangle2D viewB;
	private Ellipse2D processor;

	private Ellipse2D occupiedEll;
	private Rectangle2D occupiedRect;

	private Rectangle2D moreStatiRect;
	...
	private Color busyC = queueC.brighter().brighter();

	private Color animC = Color.RED;
	private boolean gradientF;

	private boolean viewButtonClicked = false;
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < queueLength(); i++) {
			if (i < jobs) {
				drawQueueStatus(queueLength() - i - 1, queueC, Color.black, gradientF, g2d);
			} else {
				drawQueueStatus(queueLength() - i - 1, emptyC, Color.black, gradientF, g2d);
			}
		}
	...
	private void drawQueueStatus(int i, Color startC, Color border, boolean gradientFill, Graphics2D g2d) {
		...
		g2d.draw(queue[i]);
                ...
                // Check if the state corresponds to the mean number of jobs
		if (i == queueLength() - queueMedia) {
			// draw a triangle above that represents the average
		...
		x += ELEM_WIDTH / 2.0;
		y += ELEM_HEIGHT / 2.0;
		drawCenteredText("" + (queueLength() - i), readColor(startC), null, x, y, g2d, false, true);
	}
	...
	private Point2D.Double getProcessorXY() {
		return new Point2D.Double(getElementXY(queueLength()).x, panelH * 0.4 - PROC_RAD);
	}
	...
	public void servingJob(int jobId) {
		donejobs = jobId;
	}

 ...
 * <p>Description: This thread is responsible for dispatching several simulations. If a simulation max
 * time has been specified, an inner thread will be used to stop simulation after that time. We do not rely
	...
	public void run() {
		...
		for (currentStep = 0; currentStep < pad.getNumberOfSteps(); currentStep++) {
			...
			for (int i = 0; i < results.getMeasureNumber(); i++) {
				...
				parametricAnalysisResultsModel.addSample(results.getName(i), tempValue.getLowerBound(), tempValue.getMeanValue(), tempValue.getUpperBound(), valid);
			}
			if (timer != null) {
				timer.kill();
				//simulator.killSimulation();
		...
		PAResultsWindow resWin = new PAResultsWindow(simd.getParametricAnalysisModel(), parametricAnalysisResultsModel, gui.getFileName());

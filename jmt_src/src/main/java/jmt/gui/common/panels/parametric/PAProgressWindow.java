...
public class PAProgressWindow extends JMTDialog {
	...
	public PAProgressWindow(Frame owner, AbstractAction startAction, AbstractAction pauseAction, final AbstractAction stopAction,
		...
		this.centerWindow(CommonConstants.MAX_GUI_WIDTH_WHATIF_PROGRESS, CommonConstants.MAX_GUI_HEIGHT_WHATIF_PROGRESS);
	...
	private void updateStatistics(int lastSimulationCompletedIndex) {
		...
		//for cycle used to get minimum and maximum simulation duration
		...
		int estimatedTime;
		int[] hist; // it is a vector containing the number of simulation with a duration in a certain range.
		// it is like a histogram; the algorithm uses the relative frequencies to estimate the most
	...
	/**
	 * Called when the parametric analysis is finished, not when it is stopped
	 */

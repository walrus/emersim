...
public class KMeanScatter extends JPanel {
	/**
	 * 
	 */
	...
	protected class GlassPanel extends JComponent implements MouseListener, MouseMotionListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			if ((e.getX() - UpperLeftGraph.x) >= 0 && (e.getY() - UpperLeftGraph.y) >= 0 && (e.getX() - UpperLeftGraph.x) < KMeanScatter.this.WIDTH
					&& (e.getY() - UpperLeftGraph.y) < KMeanScatter.this.HEIGHT) {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if ((e.getX() - UpperLeftGraph.x) >= 0 && (e.getY() - UpperLeftGraph.y) >= 0 && (e.getX() - UpperLeftGraph.x) < KMeanScatter.this.WIDTH
					&& (e.getY() - UpperLeftGraph.y) < KMeanScatter.this.HEIGHT && zoomming && e.getButton() == MouseEvent.BUTTON1) {
				xEnd = e.getX();
				yEnd = e.getY();
				zoomming = false;
				...
				if (xStart < xEnd && yStart < yEnd) {
					xMin = xMin + rangeX / KMeanScatter.this.WIDTH * (xStart - UpperLeftGraph.x);
					yMax = yMax - rangeY / KMeanScatter.this.WIDTH * (yStart - UpperLeftGraph.y);
					xMax = xMin + rangeX / KMeanScatter.this.WIDTH * (xEnd - xStart);
					yMin = yMax - rangeY / KMeanScatter.this.WIDTH * (yEnd - yStart);
				}
				if (xStart < xEnd && yStart > yEnd) {
					xMin = xMin + rangeX / KMeanScatter.this.WIDTH * (xStart - UpperLeftGraph.x);
					yMax = yMax - rangeY / KMeanScatter.this.WIDTH * (yEnd - UpperLeftGraph.y);
					xMax = xMin + rangeX / KMeanScatter.this.WIDTH * (xEnd - xStart);
					yMin = yMax - rangeY / KMeanScatter.this.WIDTH * (yStart - yEnd);
				}
				if (xStart > xEnd && yStart < yEnd) {
					xMin = xMin + rangeX / KMeanScatter.this.WIDTH * (xEnd - UpperLeftGraph.x);
					yMax = yMax - rangeY / KMeanScatter.this.WIDTH * (yStart - UpperLeftGraph.y);
					xMax = xMin + rangeX / KMeanScatter.this.WIDTH * (xStart - xEnd);
					yMin = yMax - rangeY / KMeanScatter.this.WIDTH * (yEnd - yStart);
				}
				if (xStart > xEnd && yStart > yEnd) {
					xMin = xMin + rangeX / KMeanScatter.this.WIDTH * (xEnd - UpperLeftGraph.x);
		...
		public void mouseDragged(MouseEvent e) {
			if ((e.getX() - UpperLeftGraph.x) >= 0 && (e.getY() - UpperLeftGraph.y) >= 0 && (e.getX() - UpperLeftGraph.x) < KMeanScatter.this.WIDTH
					&& (e.getY() - UpperLeftGraph.y) < KMeanScatter.this.HEIGHT && zoomming) {
				xEnd = e.getX();
				yEnd = e.getY();
				p.repaint();
			}
		...
		private void addListeners() {
			restore.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					first = true;
					zoomming = false;
					KMeanScatter.this.p.repaint();
					KMeanScatter.this.repaint();
				}
			});
			saveAs.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					KMeanScatter.this.showScreenShotDialog();
				}
			});

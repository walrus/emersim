...
public class KMeanSingleClusterScatter extends JPanel {
	/**
	 * 
	 */
	...
	private KMeanSingleClusterScatterPopupMenu popup = new KMeanSingleClusterScatterPopupMenu();
	...
	protected class GlassPanel extends JComponent implements MouseListener, MouseMotionListener {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if ((e.getX() - UpperLeftGraph.x) >= 0 && (e.getY() - UpperLeftGraph.y) >= 0
					&& (e.getX() - UpperLeftGraph.x) < KMeanSingleClusterScatter.this.WIDTH
					&& (e.getY() - UpperLeftGraph.y) < KMeanSingleClusterScatter.this.HEIGHT && zoomming && e.getButton() == MouseEvent.BUTTON1) {
				xEnd = e.getX();
				yEnd = e.getY();
				zoomming = false;
				...
				if (xStart < xEnd && yStart < yEnd) {
					xMin = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xStart - UpperLeftGraph.x);
					yMax = yMax - rangeY / KMeanSingleClusterScatter.this.WIDTH * (yStart - UpperLeftGraph.y);
					xMax = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xEnd - xStart);
					yMin = yMax - rangeY / KMeanSingleClusterScatter.this.WIDTH * (yEnd - yStart);
				}
				if (xStart < xEnd && yStart > yEnd) {
					xMin = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xStart - UpperLeftGraph.x);
					yMax = yMax - rangeY / KMeanSingleClusterScatter.this.WIDTH * (yEnd - UpperLeftGraph.y);
					xMax = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xEnd - xStart);
					yMin = yMax - rangeY / KMeanSingleClusterScatter.this.WIDTH * (yStart - yEnd);
				}
				if (xStart > xEnd && yStart < yEnd) {
					xMin = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xEnd - UpperLeftGraph.x);
					yMax = yMax - rangeY / KMeanSingleClusterScatter.this.WIDTH * (yStart - UpperLeftGraph.y);
					xMax = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xStart - xEnd);
					yMin = yMax - rangeY / KMeanSingleClusterScatter.this.WIDTH * (yEnd - yStart);
				}
				if (xStart > xEnd && yStart > yEnd) {
					xMin = xMin + rangeX / KMeanSingleClusterScatter.this.WIDTH * (xEnd - UpperLeftGraph.x);
		...
		public void mouseDragged(MouseEvent e) {
			if ((e.getX() - UpperLeftGraph.x) >= 0 && (e.getY() - UpperLeftGraph.y) >= 0
					&& (e.getX() - UpperLeftGraph.x) < KMeanSingleClusterScatter.this.WIDTH
					&& (e.getY() - UpperLeftGraph.y) < KMeanSingleClusterScatter.this.HEIGHT && zoomming) {
				xEnd = e.getX();
				yEnd = e.getY();
				p.repaint();
			}
	...
	protected class KMeanSingleClusterScatterPopupMenu extends JPopupMenu {
		/**
		 * 
		 */
		private static final long serialVersionUID = 1L;
		...
		private void addListeners() {
			restore.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					first = true;
					zoomming = false;
					KMeanSingleClusterScatter.this.p.repaint();
					KMeanSingleClusterScatter.this.repaint();
				}
			});
			saveAs.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					KMeanSingleClusterScatter.this.showScreenShotDialog();
				}
			});

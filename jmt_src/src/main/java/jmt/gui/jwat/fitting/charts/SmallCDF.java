...
public class SmallCDF extends JPanel {
	/**
	 * 
	 */
	...
	public SmallCDF(ModelFitting model, FittingAlgorithm engfitting) {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					plotFrame.setContentPane(new EnlargeCDF(SmallCDF.this.model, SmallCDF.this.engfitting, distribution));

					if (distribution == EXPO) {
						plotFrame.setTitle("CDF function of the selected variable against the exponential distribution");
					}
					
					if (distribution == PARETO) {
						plotFrame.setTitle("CDF function of the selected variable against the Pareto distribution");
					}

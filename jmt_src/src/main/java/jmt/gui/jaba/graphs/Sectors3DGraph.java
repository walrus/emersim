...
public class Sectors3DGraph extends JabaGraph implements MouseListener {
	...
	private double traslationX;
	private double traslationY;
			...
			public void mouseMoved(MouseEvent e) {
				if (isShown) {
					...
					if (coord != null) {
						...
						Sectors3DGraph.this.setCursor(Cursor.getPredefinedCursor(Cursor.CROSSHAIR_CURSOR));
					} else {
						coordLabel.setText("");
						coordLabel.setVisible(false);
						Sectors3DGraph.this.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
					}
				}
			}
		});

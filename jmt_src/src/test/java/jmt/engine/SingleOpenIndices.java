...
public class SingleOpenIndices {

	@Test
	public void indices01() throws Exception {
		File input = File.createTempFile("indices-01", ".xml");
		File result = File.createTempFile("indices-01-result", ".xml");
		File expected = new File(getClass().getResource("indices-01-output.xml").toURI());
		FileUtils.copyFile(new File(getClass().getResource("indices-01-input.xml").toURI()), input);
